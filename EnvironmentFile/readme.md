...
SambaMountIncludedList|The device pattern list when Samba server mounting all volume/partition devices, separated by semicolons or commas.|sd*;mmcblk*
...
SambaConfFormat|The format of the node in the configuration file for one share. ```{0}``` is the share name. ```{1}``` is the path.|[{0}]\npath = {1}\nguest ok = yes\nwritable = yes\nforce user = root\nforce group = root\n
